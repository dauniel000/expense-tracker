...
<div class="w-full">
    ...
    {#each data.incomes as income}
        <div class="w-full bg-green-900 px-4 py-2 rounded-lg flex justify-between">
            <h3>{income.job}</h3>
            <h3>{income.income} pln</h3>
        </div>
    {/each}

    <h2 class="my-2">Expenses</h2>
    {#each data.expenses as expense}
    <div class="w-full bg-red-900 px-4 py-2 rounded-lg flex justify-between">
        <h3>{expense.title}</h3>
        <h3>{expense.cost} pln</h3>
    </div>
    {/each}
    ...
    <form method="post">
        ...
        <div class="flex justify-center mt-2 gap-3">
            ...
            <input type="radio" name="income-or-income" id="income">
            <label for="expense" class="ml-5">Expense</label>
            <input type="radio" name="income-or-income" id="expense">
        </div>
